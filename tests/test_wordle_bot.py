...
def test_multiple_lines_single_message():
    ...
    import asyncio
    asyncio.run(worldleBot.analizar_mensaje(message))
    assert worldleBot.puntajes["Alice"]["total"] == 9
...
def test_multiple_players_single_line():
    ...
    import asyncio
    asyncio.run(worldleBot.analizar_mensaje(message))
    assert worldleBot.puntajes["Alice"]["total"] == 6
    assert worldleBot.puntajes["Bob"]["total"] == 6
...
def test_repeated_player_accumulates():
    ...
    import asyncio
    asyncio.run(worldleBot.analizar_mensaje(msg1))
    asyncio.run(worldleBot.analizar_mensaje(msg2))
    assert worldleBot.puntajes["Alice"]["total"] == 8
    assert worldleBot.puntajes["Alice"]["detalles"][2] == 1
    assert worldleBot.puntajes["Alice"]["detalles"][4] == 1
...
def test_multiple_messages_different_users():
    ...
    asyncio.run(worldleBot.analizar_mensaje(msg1))
    asyncio.run(worldleBot.analizar_mensaje(msg2))
    assert worldleBot.puntajes["Alice"]["total"] == 5
    assert worldleBot.puntajes["Bob"]["total"] == 3
...
def test_details_tracking_various_attempts():
    ...
    import asyncio
    for m in msgs:
        asyncio.run(worldleBot.analizar_mensaje(m))
    assert worldleBot.puntajes["Alice"]["detalles"][1] == 1
    assert worldleBot.puntajes["Alice"]["detalles"][2] == 1
    assert worldleBot.puntajes["Alice"]["detalles"][3] == 1
    assert worldleBot.puntajes["Alice"]["total"] == 15
